<?php
namespace HealthySmilesSpace\TemplateFunctions;
...
class Heading_Elements {
	...
	public static function get_page_title( $context = true ) {
		...
		if ( is_singular() ) {
			...
			$title = sprintf( esc_html__( 'Search Results for: %s', 'healthy-smiles' ), get_search_query() );

			if ( get_query_var( 'paged' ) ) {
				/* translators: Paged search query title part. 1: Page number */
				$title .= sprintf( esc_html__( '&nbsp;&ndash; Page %s', 'healthy-smiles' ), get_query_var( 'paged' ) );
			}
		} elseif ( ! $context ) {
			if ( is_category() ) {
				...
				$title = get_the_date( _x( 'Y', 'yearly archives date format', 'healthy-smiles' ) );
			} elseif ( is_month() ) {
				$title = get_the_date( _x( 'F Y', 'monthly archives date format', 'healthy-smiles' ) );
			} elseif ( is_day() ) {
				$title = get_the_date( _x( 'F j, Y', 'daily archives date format', 'healthy-smiles' ) );
	...
	public static function get_page_breadcrumbs( $sep_text = ' / ' ) {
		...
		if ( '' !== $out ) {
			...
			$out .= '<span>' . esc_html__( 'Search results for', 'healthy-smiles' ) . ': "' . esc_html( get_search_query() ) . '"</span>';
			...
			if ( is_tax( 'post_format', 'post-format-gallery' ) ) {
				$out .= '<span>' . esc_html_x( 'Galleries', 'post format archive title', 'healthy-smiles' ) . '</span>';
			} elseif ( is_tax( 'post_format', 'post-format-image' ) ) {
				$out .= '<span>' . esc_html_x( 'Images', 'post format archive title', 'healthy-smiles' ) . '</span>';
			} elseif ( is_tax( 'post_format', 'post-format-video' ) ) {
				$out .= '<span>' . esc_html_x( 'Videos', 'post format archive title', 'healthy-smiles' ) . '</span>';
			} elseif ( is_tax( 'post_format', 'post-format-audio' ) ) {
				$out .= '<span>' . esc_html_x( 'Audio', 'post format archive title', 'healthy-smiles' ) . '</span>';
			}
		} elseif ( is_post_type_archive() ) {
			...
			$out .= '<span>' . esc_html__( 'No breadcrumbs', 'healthy-smiles' ) . '</span>';
		}

		$out = apply_filters( 'cmsmasters_page_breadcrumbs_filter_after', $out );

		return '<a href="' . esc_url( home_url( '/' ) ) . '">' . esc_html__( 'Home', 'healthy-smiles' ) . '</a>' . $sep . $out;
	}
