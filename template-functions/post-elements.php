...
use HealthySmilesSpace\Core\Utils\Excerpt;
use HealthySmilesSpace\Core\Utils\Utils;
use HealthySmilesSpace\TemplateFunctions\Post_Media;
...
class Post_Elements {
	...
	public static function get_post_author( $atts = array() ) {
		...
		$out = '<div class="' . esc_attr( $parent_class ) . '-author">' .
			...
			'<a href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '" title="' . esc_attr__( 'Posts by', 'healthy-smiles' ) . ' ' . esc_attr( get_the_author_meta( 'display_name' ) ) . '" class="vcard author">' .
	...
	public static function get_post_comments( $atts = array() ) {
		...
		if ( 'icon' === $type ) {
			$out .= '<a class="cmsmasters-theme-icon-comment cmsmasters-' . esc_attr( $icon_position ) . '" href="' . esc_url( get_comments_link() ) . '" title="' . esc_attr__( 'Comment on', 'healthy-smiles' ) . ' ' . esc_attr( get_the_title() ) . '">' .
				'<span>' . esc_html( get_comments_number() ) . '</span>' .
			'</a>';
		} else {
			$out .= esc_html__( 'Comments', 'healthy-smiles' ) . ' (' .
			'<a href="' . esc_url( get_comments_link() ) . '" title="' . esc_attr__( 'Comment on', 'healthy-smiles' ) . ' ' . esc_attr( get_the_title() ) . '">' .
		...
		foreach ( $elements as $element ) {
			if ( 'categories' === $element ) {
				...
				$out .= self::get_post_terms( array(
					'parent_class' => $parent_class,
					'hierarchical' => false,
					'before' => '<span>' . esc_html__( 'In', 'healthy-smiles' ) . ': </span>',
					'sep' => ', ',
					'after' => '',
				) );
			}
