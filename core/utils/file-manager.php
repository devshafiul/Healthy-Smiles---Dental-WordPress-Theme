...
class File_Manager {
	...
	public static function upload_and_extract_zip( $file_path ) {
		if ( ! did_action( 'elementor/loaded' ) || ! class_exists( 'Cmsmasters_Elementor_Addon' ) ) {
			return new \WP_Error( 'required_plugins_activation', esc_html__( 'Required plugins not activated', 'healthy-smiles' ) );
		}
		...
		if ( 0 === count( $source_files ) ) {
			return new \WP_Error( 'incompatible_archive', esc_html__( 'Incompatible archive', 'healthy-smiles' ) );
		} elseif (
	...
	public static function get_css_template_assets_url( $file_name, $relative_url = null, $add_min_suffix = 'default', $add_direction_suffix = false, $path_prefix = '' ) {
		if ( Utils::has_custom_breakpoints() ) {
			...
			$responsive_file = new ElementorResponsiveFilesFrontend( "healthy-smiles-{$file}", self::get_responsive_css_path( $path_prefix ) . $file );
	...
	public static function get_upload_path( $dir_name = '', $file_name = '', $url = false, $slash = false ) {
		...
		$out = str_replace( '\\', '/', $out . '/cmsmasters/healthy-smiles' );
	...
	public static function write_file( $content, $dir_name, $filename, $filetype = 'css' ) {
		...
		if ( false === $is_dir ) {
			update_option( "cmsmasters_healthy-smiles_{$dir_name}_exists", 'no' );
		...
		if ( true === $created ) {
			update_option( "cmsmasters_healthy-smiles_{$dir_name}_exists", 'yes' );
		}
	}
	...
	public static function delete_uploaded_dir( $dir_name = '' ) {
		...
		delete_option( "cmsmasters_healthy-smiles_{$dir_name}_exists" );
	}
