<?php
namespace HealthySmilesSpace\Core;

use HealthySmilesSpace\Admin\Admin;
use HealthySmilesSpace\Core\Traits\Singleton;
use HealthySmilesSpace\Core\Utils\File_Manager;
use HealthySmilesSpace\Core\Utils\Utils;
use HealthySmilesSpace\Kits\Module as Kits;
use HealthySmilesSpace\Modules\Modules;
use HealthySmilesSpace\TemplateFunctions\General_Elements;
use HealthySmilesSpace\TemplateFunctions\Main_Elements;
use HealthySmilesSpace\ThemeConfig\Theme_Config;
use HealthySmilesSpace\ThemeConfig\Theme_Plugins;
...
class Core {
	...
	public function get_responsive_stylesheet_templates( $templates ) {
		...
		foreach ( $templates_paths as $template_path ) {
			$file_name = 'healthy-smiles-' . basename( $template_path );
	...
	public function enqueue_frontend_assets() {
		...
		wp_localize_script( 'healthy-smiles-frontend', 'cmsmasters_localize_vars', array(
	...
	public function register_widget_areas() {
		...
		register_sidebar(
			array(
				'name' => esc_html__( 'Sidebar', 'healthy-smiles' ),
				'id' => 'sidebar_default',
				'description' => esc_html__( 'Widgets in this area will be shown in all left and right sidebars.', 'healthy-smiles' ),
		...
		register_sidebar(
			array(
				'name' => esc_html__( 'Archive Sidebar', 'healthy-smiles' ),
				'id' => 'sidebar_archive',
				'description' => esc_html__( 'Widgets in this area will be shown in all left and right sidebars on archives pages.', 'healthy-smiles' ),
				'before_widget' => '<aside id="%1$s" class="widget %2$s">',
				'after_widget' => '</aside>',
				'before_title' => '<h6 class="widgettitle">',
				'after_title' => '</h6>',
		...
		register_sidebar(
			array(
				'name' => esc_html__( 'Search Sidebar', 'healthy-smiles' ),
				'id' => 'sidebar_search',
				'description' => esc_html__( 'Widgets in this area will be shown in all left and right sidebars on archives pages.', 'healthy-smiles' ),
				'before_widget' => '<aside id="%1$s" class="widget %2$s">',
				'after_widget' => '</aside>',
		...
		register_sidebar(
			array(
				'name' => esc_html__( 'Footer 1', 'healthy-smiles' ),
				'id' => 'footer-1',
				'description' => esc_html__( 'Widgets in this area will be shown in footer area.', 'healthy-smiles' ),
				'before_widget' => '<aside id="%1$s" class="widget %2$s">',
				'after_widget' => '</aside>',
				'before_title' => '<h6 class="widgettitle">',
				'after_title' => '</h6>',
		...
		register_sidebar(
			array(
				'name' => esc_html__( 'Footer 2', 'healthy-smiles' ),
				'id' => 'footer-2',
				'description' => esc_html__( 'Widgets in this area will be shown in footer area.', 'healthy-smiles' ),
				'before_widget' => '<aside id="%1$s" class="widget %2$s">',
				'after_widget' => '</aside>',
		...
		register_sidebar(
			array(
				'name' => esc_html__( 'Footer 3', 'healthy-smiles' ),
				'id' => 'footer-3',
				'description' => esc_html__( 'Widgets in this area will be shown in footer area.', 'healthy-smiles' ),
				'before_widget' => '<aside id="%1$s" class="widget %2$s">',
				'after_widget' => '</aside>',
		...
		register_sidebar(
			array(
				'name' => esc_html__( 'Footer 4', 'healthy-smiles' ),
				'id' => 'footer-4',
				'description' => esc_html__( 'Widgets in this area will be shown in footer area.', 'healthy-smiles' ),
				'before_widget' => '<aside id="%1$s" class="widget %2$s">',
				'after_widget' => '</aside>',
		...
		register_sidebar(
			array(
				'name' => esc_html__( 'Footer 5', 'healthy-smiles' ),
				'id' => 'footer-5',
				'description' => esc_html__( 'Widgets in this area will be shown in footer area.', 'healthy-smiles' ),
				'before_widget' => '<aside id="%1$s" class="widget %2$s">',
				'after_widget' => '</aside>',
	...
	public function register_translations() {
		load_theme_textdomain( 'healthy-smiles', get_template_directory() . '/theme-config/languages' );
	}
	...
	public function register_nav_menu_locations() {
		register_nav_menus(
			array(
				'header_top_nav' => esc_html__( 'Header Top Navigation', 'healthy-smiles' ),
				'header_mid_nav' => esc_html__( 'Header Middle Navigation', 'healthy-smiles' ),
				'header_bot_nav' => esc_html__( 'Header Bottom Navigation', 'healthy-smiles' ),
				'footer_nav' => esc_html__( 'Footer Navigation', 'healthy-smiles' ),
			)
		);
	}
	...
	public function filter_password_form() {
		$output = '<p>' . esc_html__( 'This content is password protected. To view it please enter your password below:', 'healthy-smiles' ) . '</p>' .
		'<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form cmsmasters-post-password-form" method="post">' .
			'<input name="post_password" type="password" size="20" placeholder="' . esc_attr__( 'Password', 'healthy-smiles' ) . '" />' .
			'<input type="submit" name="Submit" value="' . esc_attr_x( 'Enter', 'post password form', 'healthy-smiles' ) . '" />' .
		'</form>';
