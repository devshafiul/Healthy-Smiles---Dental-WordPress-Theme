...
use HealthySmilesSpace\TemplateFunctions\Post_Elements;
...
if ( is_search() && ! have_posts() ) {
	echo '<div class="' . $parent_class . '__no-posts">' .
		'<h3 class="' . $parent_class . '__no-posts-title">' . esc_html__( 'Nothing found', 'healthy-smiles' ) . '</h3>' .
		'<div class="' . $parent_class . '__no-posts-description">' .
			'<p>' . esc_html__( 'Sorry, no posts matched your criteria. Please try another search.', 'healthy-smiles' ) . '</p>' .
			'<p>' . esc_html__( 'You might want to consider some of our suggestions to get better results:', 'healthy-smiles' ) . '</p>' .
			'<ul>' .
				'<li>' . esc_html__( 'Check your spelling.', 'healthy-smiles' ) . '</li>' .
				'<li>' . esc_html__( 'Try a similar keyword, for example: tablet instead of laptop.', 'healthy-smiles' ) . '</li>' .
				'<li>' . esc_html__( 'Try using more than one keyword.', 'healthy-smiles' ) . '</li>' .
			'</ul>' .
		'</div>' .
	'</div>';
}
